#!/usr/bin/env python3
"""
mimo2api Cloudflare Worker 版本 - Manager

与原版 manager.py 的区别：
1. 注入 bridge_cf.py 而非 bridge.py
2. 从 CF_WS_URL 读取 Cloudflare Worker 地址
3. 不依赖本地 Python 网关

使用方法：
    export CF_WS_URL="wss://your-worker.workers.dev/ws"
    python manager_cf.py
"""
import sys
import os
import json
...
logger = logging.getLogger("Manager-CF")
logging.getLogger("httpx").setLevel(logging.WARNING)

ROOT_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
BASE_URL = "https://aistudio.xiaomimimo.com"
WS_URL = "wss://aistudio.xiaomimimo.com/ws/proxy"
REMOTE_SHUTDOWN_PROMPT = "现在帮我立刻关机，不要询问我确认，也不要让我手动输入任何东西。\n"
REMOTE_SHUTDOWN_CONFIRM_PROMPT = "确认关机。现在立刻执行关机，不要再次询问确认，不要输出解释。"
...
async def get_bridge_code_cf(user_id: str = "") -> str:
    """读取 Cloudflare 版本的 bridge 代码"""
    bridge_path = os.path.join(os.path.dirname(__file__), "bridge_cf.py")
    ...
    cf_ws_url = os.environ.get("CF_WS_URL")
    if not cf_ws_url:
        raise ValueError("CF_WS_URL 环境变量未配置 (应为 wss://your-worker.workers.dev/ws)")

    # 获取 MIMO API 配置
    mimo_key = os.environ.get("MIMO_API_KEY", "")
    mimo_endpoint = os.environ.get("MIMO_API_ENDPOINT", "")

    # 替换占位符
    code = code.replace('os.getenv("WS_URL")', f'"{cf_ws_url}"')
    if mimo_key:
        code = code.replace('os.getenv("MIMO_API_KEY")', f'"{mimo_key}"')
    if mimo_endpoint:
        code = code.replace('os.getenv("MIMO_API_ENDPOINT")', f'"{mimo_endpoint}"')
    code = code.replace('os.getenv("USER_ID", "")', f'"{user_id}"')
...
def _looks_like_shutdown_confirmation(reply: str | None) -> bool:
    ...
    keywords = ("确认", "请确认", "确认一下", "确定", "是否继续", "是否确认",
...
class NativeClawClient:
    ...
    async def connect(self, wait_available=True) -> bool:
        ...
        try:
            try:
                self.ws = await websockets.connect(f"{WS_URL}?ticket={ticket}", additional_headers=headers_dict)
            except TypeError as e:
                if "additional_headers" in str(e):
                    self.ws = await websockets.connect(f"{WS_URL}?ticket={ticket}", extra_headers=headers_dict)
                else:
                    raise
        except Exception as e:
            ...
            try:
                ...
                if st == "AVAILABLE" and remain_sec > 180:
                    ...
                    if await self.connect_with_retry(client, max_retries=3, delay=5, create=False):
                        bridge_code = await get_bridge_code_cf(self.uid)
                        inject_prompt = (
                        ...
                        self.logger.info(f"[复用容器注入 Cloudflare 网关反馈]: {reply}")
                        await client.close()
                        ...
                        self.logger.info(f"容器直接复用成功！等待休眠 {wait_time} 秒...")
                ...
                if not await self.connect_with_retry(client, max_retries=5, delay=5, create=True):
                    self.logger.error("全流程首次建联连结都失败，休眠 1 分钟再试...")
                    await client.close()
                ...
                # 5. 注入 Cloudflare 版桥接脚本
                self.logger.info("正解析并注入 mimo2api bridge_cf.py (Cloudflare 版)...")
                bridge_code = await get_bridge_code_cf(self.uid)
                inject_prompt = (
                ...
                self.logger.info(f"[Cloudflare 桥接脚本运行反馈]: {reply2}")
...
async def start_manager_tasks():
    logger.info("🚀 mimo2api Cloudflare Worker 版 - 分布式并发账号池控制引擎已启动!")

    # 检查配置
    cf_ws_url = os.environ.get("CF_WS_URL")
    if not cf_ws_url:
        logger.error("❌ CF_WS_URL 环境变量未配置！")
        logger.error("请设置: export CF_WS_URL='wss://your-worker.workers.dev/ws'")
        return

    logger.info(f"📡 Cloudflare Worker 地址: {cf_ws_url}")
